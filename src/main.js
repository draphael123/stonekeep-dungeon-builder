...
const meadowMaterial=new THREE.MeshStandardMaterial({color:0x354536,roughness:1}),dryGrassMaterial=new THREE.MeshStandardMaterial({color:0x4b4934,roughness:1}),waterMaterial=new THREE.MeshStandardMaterial({color:0x315a5d,roughness:.24,metalness:.12,transparent:true,opacity:.82}),flowerMaterial=new THREE.MeshStandardMaterial({color:0xb99a65,roughness:.9});
function shingleMaterial(base,line){const c=document.createElement('canvas');c.width=c.height=256;const x=c.getContext('2d');x.fillStyle=base;x.fillRect(0,0,256,256);for(let y=0;y<256;y+=32){x.fillStyle=y%64?line:'#00000022';x.fillRect(0,y,256,3);for(let col=-16;col<256;col+=32){x.fillRect(col+(y%64?16:0),y,2,32);}}for(let i=0;i<700;i++){x.fillStyle=`rgba(255,255,255,${Math.random()*.025})`;x.fillRect(Math.random()*256,Math.random()*256,2,2);}const map=new THREE.CanvasTexture(c);map.colorSpace=THREE.SRGBColorSpace;map.wrapS=map.wrapT=THREE.RepeatWrapping;map.repeat.set(3,3);return new THREE.MeshStandardMaterial({map,color:0xffffff,roughness:.92});}
const roofShingles=shingleMaterial('#663a2f','#3d241f'),hallShingles=shingleMaterial('#403f43','#24252a');
...
function buildSurfaceTerrain(){
  terrainGroup.clear();
  for(let i=0;i<14;i++){const angle=i*2.18,radius=14+(i%5)*4,geo=new THREE.CircleGeometry(3.2+(i%4)*1.15,18),pos=geo.attributes.position;for(let v=1;v<pos.count;v++){const irregular=.82+Math.sin(v*4.7+i*2.1)*.16;pos.setXY(v,pos.getX(v)*irregular,pos.getY(v)*irregular);}const mat=(i%3?meadowMaterial:dryGrassMaterial).clone();mat.transparent=true;mat.opacity=.22;mat.depthWrite=false;const patch=new THREE.Mesh(geo,mat);patch.rotation.x=-Math.PI/2;patch.position.set(Math.cos(angle)*radius,-.065,Math.sin(angle)*radius);patch.scale.y=.55+(i%3)*.2;patch.receiveShadow=true;terrainGroup.add(patch);}
  ...
  for(let i=0;i<6;i++){const a=i*1.91,r=17+(i%2)*7,x=Math.cos(a)*r,z=Math.sin(a)*r;addCylinder(terrainGroup,.3,.42,[x,.21,z],mats.wood,10);for(let j=0;j<3;j++){const root=addBox(terrainGroup,[.55,.08,.1],[x+Math.cos(j*2.1)*.28,.06,z+Math.sin(j*2.1)*.28],mats.wood);root.rotation.y=-j*2.1;}}
  const pond=new THREE.Mesh(new THREE.CircleGeometry(5.2,32),waterMaterial);pond.rotation.x=-Math.PI/2;pond.scale.y=.58;pond.position.set(-22,-.035,18);terrainGroup.add(pond);for(let i=0;i<18;i++){const a=i/18*Math.PI*2,x=-22+Math.cos(a)*5.1,z=18+Math.sin(a)*3,stone=addBox(terrainGroup,[.35+(i%3)*.16,.18,.3+(i%2)*.2],[x,.02,z],mats.wallTop);stone.rotation.y=a;}
  for(let i=0;i<9;i++){const a=i/9*Math.PI*2,h=5+(i%4)*1.5,hill=new THREE.Mesh(new THREE.ConeGeometry(8+(i%3)*2,h,14),i%2?meadowMaterial:mats.moss);hill.position.set(Math.cos(a)*54,-1+ h/2,Math.sin(a)*54);hill.scale.z=.65;hill.receiveShadow=true;terrainGroup.add(hill);}
  const ruinX=25,ruinZ=-18;for(const [x,z,h] of [[0,0,2.4],[2.5,0,1.5],[0,2.4,1.8],[2.5,2.4,.9]]){addBox(terrainGroup,[.65,h,.65],[ruinX+x,h/2,ruinZ+z],mats.wall);addBox(terrainGroup,[.82,.12,.82],[ruinX+x,h+.04,ruinZ+z],mats.wallTop);}addBox(terrainGroup,[3.1,.38,.45],[ruinX+1.25,1.8,ruinZ],mats.wallTop);
}
buildSurfaceTerrain();
...
function addSurfaceBuilding(group,room){
  ...
  const wallH=room.role==='watchtower'?4.2:2.35,plaster=new THREE.MeshStandardMaterial({color:room.role==='hall'?0x71624f:0x8a785e,roughness:.98}),wallMat=['forge','watchtower'].includes(room.role)?mats.wall:plaster,windowGlow=new THREE.MeshStandardMaterial({color:0xe4a95b,emissive:0xb76522,emissiveIntensity:1.8,roughness:.4});
  ...
  const roof=new THREE.Group();roof.userData.surfaceRoof=true;roof.userData.roomId=room.id;const roofMat=room.role==='hall'?hallShingles:roofShingles,rise=Math.min(1.65,w*.28),slope=Math.sqrt((w*.5+.28)**2+rise**2),angle=Math.atan2(rise,w*.5+.28);
  const left=addBox(roof,[slope,.13,d+.5],[cx-w*.25,wallH+rise*.5,cz],roofMat);left.rotation.z=angle;const right=addBox(roof,[slope,.13,d+.5],[cx+w*.25,wallH+rise*.5,cz],roofMat);right.rotation.z=-angle;
  addBox(roof,[.16,.18,d+.62],[cx,wallH+rise,cz],room.role==='hall'?mats.wallTop:mats.wood);
  const gableShape=new THREE.Shape();gableShape.moveTo(-w*.5,0);gableShape.lineTo(w*.5,0);gableShape.lineTo(0,rise);gableShape.closePath();const gableMat=new THREE.MeshStandardMaterial({color:room.role==='hall'?0x5f5b53:0x79674f,roughness:.96,side:THREE.DoubleSide});for(const z of [room.z*CELL-.01,(room.z+room.d)*CELL+.01]){const gable=new THREE.Mesh(new THREE.ShapeGeometry(gableShape),gableMat);gable.position.set(cx,wallH,z);if(z>(room.z+room.d/2)*CELL)gable.rotation.y=Math.PI;roof.add(gable);}
...
function showNeedView(kind){infoGroup.clear();document.querySelectorAll('[data-need]').forEach(b=>b.classList.toggle('active',b.dataset.need===kind));if(kind==='none'){updateSimHUD();return;}for(const r of state.rooms.filter(x=>(x.layer||'underground')===state.layer)){let score=.5;if(kind==='food')score=['farm','orchard','kitchen'].includes(r.role)?.95:.18;if(kind==='jobs')score=['farm','orchard','forge','lumberyard','storehouse','market'].includes(r.role)?.9:.25;if(kind==='morale')score=['cottage','well','market','hall','tavern'].includes(r.role)?.9:.3;const color=new THREE.Color().setHSL(score*.32,.75,.48),m=new THREE.Mesh(new THREE.BoxGeometry(r.w*CELL+.18,.05,r.d*CELL+.18),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.42,depthWrite:false}));m.position.set((r.x+r.w/2)*CELL,.42,(r.z+r.d/2)*CELL);infoGroup.add(m);}document.querySelector('#alerts strong').textContent=kind==='food'?'Green areas provide food; red areas lack access.':kind==='jobs'?'Green buildings provide useful work.':'Green spaces support villager morale.';}
...
document.querySelectorAll('[data-build-category]').forEach(tab=>tab.onclick=()=>{document.querySelectorAll('[data-build-category]').forEach(x=>x.classList.toggle('active',x===tab));const cat=tab.dataset.buildCategory,groups={essential:['hall','cottage','farm','well'],production:['forge','lumberyard','storehouse','market','orchard','watchtower'],infrastructure:['road','cobble','boardwalk','fence']};document.querySelectorAll('#surfaceBlueprints .room-type').forEach(b=>b.style.display=cat==='all'||groups[cat]?.includes(b.dataset.roomRole)?'':'none');});
document.querySelectorAll('[data-need]').forEach(b=>b.onclick=()=>showNeedView(b.dataset.need));
document.querySelector('[data-build-category="essential"]').click();
...
function tick(){
  ...
  if(state.mode==='build'){
    ...
    if(m.lengthSq()){const cx0=Math.floor(exploreCamera.position.x/CELL),cz0=Math.floor(exploreCamera.position.z/CELL),next=exploreCamera.position.clone().addScaledVector(m.normalize(),speed*terrainSpeedAt(cx0,cz0)*dt),cx=Math.floor(next.x/CELL),cz=Math.floor(next.z/CELL);if(state.layer==='surface'){next.x=Math.max(-48,Math.min(48,next.x));next.z=Math.max(-48,Math.min(48,next.z));if(canWalkSurface(exploreCamera.position,next))exploreCamera.position.copy(next);}else if(occupiedMap().has(cellKey(cx,cz)))exploreCamera.position.copy(next);}
